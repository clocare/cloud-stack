...
resource "openstack_compute_instance_v2" "k8s_controller" {
  count           = var.controller_count
  name            = "${var.cluster_name}-k8s_controller"
...
resource "openstack_compute_instance_v2" "k8s_worker" {
  ...
  name            = "${var.cluster_name}-k8s_worker"
  flavor_id       = var.flavor_id
  image_id        = var.image_id
  key_pair        = openstack_compute_keypair_v2.k8s.name
  security_groups = [openstack_networking_secgroup_v2.k8s.name]
...
resource "openstack_networking_floatingip_v2" "k8s_controller" {
  count = var.controller_count
  pool  = var.floatingip_pool
}

resource "openstack_networking_floatingip_v2" "k8s_worker" {
  count = var.worker_count
  pool  = var.floatingip_pool
}
...
resource "openstack_compute_floatingip_associate_v2" "k8s_controller" {
  count       = var.controller_count
  floating_ip = openstack_networking_floatingip_v2.k8s_controller[count.index].address
...
resource "openstack_compute_floatingip_associate_v2" "k8s_worker" {
  count       = var.worker_count
  floating_ip = openstack_networking_floatingip_v2.k8s_worker[count.index].address
